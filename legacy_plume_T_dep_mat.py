...
# %%
air_material = TemperatureDependentMaterial(
    mesh=sub_mesh,
    T_ref=T_ambient,
    mu_ref=mu_air,
    cp_ref=cp_air,
...
# rho_func = fenics.Function(V0)
# beta_func = fenics.Function(V0)
# f_b_func = fenics.Function(V0)

T_DG0 = fenics.Function(V0)
...
mu   = air_material.mu
Pr   = air_material.Pr
rho  = air_material.rho
beta = air_material.beta

gvec = fenics.Constant((0.0, -1.0))

T_ref = fenics.Constant(T_ambient)

f_b = rho * beta * (T - T_ref) * gvec
...
for it in range(max_it):
    air_material.update(T_old)   # updates DG0 mu/Pr/... on sub_mesh
