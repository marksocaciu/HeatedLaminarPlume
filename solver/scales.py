...
class NondimScales:
    ...
    # --- legacy viscous solver scales ---
    Uref_viscous: Optional[float] = None
    Pref: Optional[float] = None

    # --- Kis/Herwig ABE scales ---
    Uref_abe: Optional[float] = None
    Pref_abe: Optional[float] = None
    mu_abe: Optional[float] = None
    kappa_abe: Optional[float] = None
    Ec_abe: Optional[float] = None
    fEc_abe: Optional[float] = None
    Ra_abe: Optional[float] = None
...
def compute_nondimensional_scales(experiment) -> NondimScales:
    ...
    # Existing finite-wire flux temperature scale
    dTref = qsurf * Lref / k

    # Existing legacy viscous velocity scale
    Uref_viscous = nu / Lref

    # Dimensionless groups
    Pr = nu / alpha
    Gr = g * beta * dTref * Lref**3 / nu**2
    Ra = Gr * Pr

    # Kis/Herwig natural-convection velocity scale
    Uref_abe = math.sqrt(g * beta * dTref * Lref)

    # Pressure scales
    Pref_viscous = rho * Uref_viscous**2
    Pref_abe = rho * Uref_abe**2

    # Kis coefficients in the nondimensional equations
    mu_abe = 1.0 / math.sqrt(Gr)
    kappa_abe = 1.0 / (Pr * math.sqrt(Gr))

    # Perturbation and Eckert numbers
    Tref = float(experiment.initial_conditions.temperature)
    e = dTref / Tref

    Ec_viscous = Uref_viscous**2 / (cp * dTref)
    fEc_viscous = Ec_viscous / e

    Ec_abe = Uref_abe**2 / (cp * dTref)
    fEc_abe = Ec_abe / e
...
def compute_heat_flux_dim(T_dim, k_dim):
    """
    Conductive heat flux q = -k grad(T) [W/m^2].

    IMPORTANT:
    The target function spaces are built on the mesh of T_dim.
    Do not pass sub_mesh_dim here unless T_dim also lives on sub_mesh_dim.
    """
    mesh = T_dim.function_space().mesh()
