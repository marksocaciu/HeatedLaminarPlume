...
def compute_fixed_eta_entrainment_diagnostics(
    ...
    theta_line = float(q_input_per_length) / (float(rho) * float(cp) * nu) if np.isfinite(nu) and nu > 0.0 else np.nan
    ...
    for i, (yy, half) in enumerate(zip(y_sorted, xh_clipped)):
        ...
        Gr_h = (float(g) * float(beta) * theta_line * h_eta**3 / nu**2) if h_eta > 0.0 and np.isfinite(theta_line) and np.isfinite(nu) and nu > 0.0 else np.nan
...
def main() -> None:
    ...
    if entrainment_rows:
        ...
        h = entrainment_df["height_above_wire_m"].to_numpy()

        plt.figure(figsize=(7, 4.5))
        plt.plot(
            h,
            entrainment_df["effective_entrainment_velocity_from_mass_m_per_s"],
            label="from mass-flux derivative",
        )
        plt.plot(
            h,
            entrainment_df["edge_inflow_mean_vertical_edge_m_per_s"],
            label="mean vertical-edge inflow",
        )
        plt.plot(
            h,
            entrainment_df["edge_inflow_mean_normal_m_per_s"],
        ...
        plt.plot(
            h,
            entrainment_df["d_mass_flux_dy_kg_per_s_per_m2"],
            label="dM/dy",
        )
        plt.axhline(0.0, linestyle="--", linewidth=1)
        plt.xlabel("Height above wire [m]")
