...
def configure_plot_style(font_size: float = 12.0) -> None:
    matplotlib.rcParams.update({
        ...
        # Export with a tight bounding box so labels, tick labels and outside
        # legends are not clipped in saved images.
        "savefig.bbox": "tight",
...
def compute_htff26_dimensionless_scales(*, q_input_per_length, k, mu, rho, cp, beta, g, d, T_inf):
    """Return the nondimensionalization used in HTFF26_Paper.pdf.

    Length scale: cylinder diameter d.
    Temperature scale: DeltaT = Qdot/(k Pr) = Qdot/(mu cp).
    Velocity scale: u_r = sqrt(g d beta DeltaT).
    Pressure scale: rho u_r^2.
    Time scale: d/u_r.
    """
    out = {
        "length_scale_d_m": float(d) if d is not None else np.nan,
        "DeltaT_ref_K": np.nan,
        "u_ref_m_per_s": np.nan,
        "time_scale_s": np.nan,
        "pressure_scale_Pa": np.nan,
        "Pr": np.nan,
        "Gr_d": np.nan,
        "Ec_tilde": np.nan,
        "nu_m2_per_s": np.nan,
        "alpha_m2_per_s": np.nan,
        "enabled": False,
    }
    try:
        q = float(q_input_per_length)
        k = float(k)
        mu = float(mu)
        rho = float(rho)
        cp = float(cp)
    ...
    if not (q > 0.0 and k > 0.0 and mu > 0.0 and rho > 0.0 and cp > 0.0 and beta > 0.0 and g > 0.0 and d > 0.0):
        return out

    nu = mu / rho
    alpha = k / (rho * cp)
    Pr = nu / alpha
    DeltaT = q / (k * Pr)   # identical to q/(mu*cp)
    uref = math.sqrt(g * d * beta * DeltaT)
    out.update({
        "DeltaT_ref_K": DeltaT,
        "u_ref_m_per_s": uref,
        "time_scale_s": d / uref if uref > 0.0 else np.nan,
        "pressure_scale_Pa": rho * uref**2,
        "Pr": Pr,
        "Gr_d": g * beta * DeltaT * d**3 / nu**2,
        "Ec_tilde": uref**2 * T_inf / (cp * DeltaT**2),
        "nu_m2_per_s": nu,
...
def thesis_figsize(width_in: Optional[float] = None, height_in: Optional[float] = None) -> Tuple[float, float]:
    """Return a figure size with a strict 16:10 canvas by default.

    This controls the saved figure canvas aspect ratio, not the data aspect
    ratio inside the axes.  For example, width=6.4 gives height=4.0.
    """
    width = 6.4 if width_in is None else float(width_in)
    height = width * 10.0 / 16.0 if height_in is None else float(height_in)
    return (width, height)
...
def save_figure_full(fig, path: Path, *, dpi: int = 220) -> None:
    """Save a Matplotlib figure without clipping labels, ticks or outside legends.

    The script uses legends outside the right edge of many axes.  Plain
    ``savefig`` can clip those artists, and ``tight_layout`` can fail for
    small figures.  This helper first gives the axes enough internal room,
    then exports with a tight bounding box so all visible artists are kept.
    """
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning)
        try:
            fig.tight_layout(rect=(0.0, 0.0, 0.84, 1.0), pad=1.2)
        except Exception:
            # Fallback for figures where tight_layout cannot solve the geometry.
            try:
                fig.subplots_adjust(left=0.16, right=0.78, bottom=0.16, top=0.94)
            except Exception:
                pass
    fig.savefig(path, dpi=dpi, bbox_inches="tight", pad_inches=0.08)


def save_current_figure_full(path: Path, *, dpi: int = 220) -> None:
    save_figure_full(plt.gcf(), path, dpi=dpi)
...
def finite_difference_sorted(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    """Return dy/dx on the original ordering, using only finite sorted samples."""
    ...
    xs = x[idxs]
    ys = y[idxs]
    # Remove duplicate x values, because np.gradient requires a strictly varying coordinate.
    ...
    if xs.size < 2:
        return out
    if xs.size == 2:
        dydx = np.full(2, (ys[1] - ys[0]) / (xs[1] - xs[0]), dtype=float)
    else:
...
def compute_fixed_eta_entrainment_diagnostics(
    Ti,
    uxi,
    uyi,
    y_levels: np.ndarray,
    eta_half_width: float,
    eta_origin_y_m: float,
    wire_y_m: float,
    xmin: float,
    xmax: float,
    rho: float,
    ...
    """Quantify plume entrainment through a fixed-|eta| core boundary.

    The diagnostic reports two complementary estimates:
      * an integral estimate, v_e = (1/(2*rho))*dM/dy, where
        M(y)=∫_{-x_e}^{x_e} rho*u_y dx is the signed vertical mass flux;
      * a direct boundary estimate from the lateral velocity at x=±x_e and the
        normal flux through the moving sidewalls x=±x_e(y).

    Positive entrainment means fluid enters the selected plume core.
    """
    ...
    theta_line = float(q_input_per_length) / (float(rho) * float(cp) * nu) if np.isfinite(nu_htff26) and nu_htff26 > 0.0 else np.nan
    y_sorted = np.asarray(sorted(float(y) for y in y_levels if np.isfinite(y)), dtype=float)
    xh = eta_halfwidth_at_y(
        y_sorted,
        eta_abs=float(eta_half_width),
        eta_origin_y=float(eta_origin_y_m),
        q_input_per_length=float(q_input_per_length),
        rho=float(rho), cp=float(cp), mu=float(mu), beta=float(beta), g=float(g),
    )
    # Keep the integration inside the sampled mesh. This prevents interpolator
    # masking at the rectangular side boundaries from dominating dM/dy.
    max_left = abs(float(xmin))
    max_right = abs(float(xmax))
    max_hw = 0.999 * min(max_left, max_right)
    xh_clipped = np.where(np.isfinite(xh), np.minimum(xh, max_hw), np.nan)
    dxh_dy = finite_difference_sorted(y_sorted, xh_clipped)

    mass_signed = np.full_like(y_sorted, np.nan, dtype=float)
    mass_upward = np.full_like(y_sorted, np.nan, dtype=float)
    mass_downward = np.full_like(y_sorted, np.nan, dtype=float)
    volume_signed = np.full_like(y_sorted, np.nan, dtype=float)
    mom_positive = np.full_like(y_sorted, np.nan, dtype=float)

    edge_ux_left = np.full_like(y_sorted, np.nan, dtype=float)
    edge_ux_right = np.full_like(y_sorted, np.nan, dtype=float)
    edge_uy_left = np.full_like(y_sorted, np.nan, dtype=float)
    edge_uy_right = np.full_like(y_sorted, np.nan, dtype=float)
    edge_inflow_left_vertical_edge = np.full_like(y_sorted, np.nan, dtype=float)
    edge_inflow_right_vertical_edge = np.full_like(y_sorted, np.nan, dtype=float)
    edge_inflow_left_normal = np.full_like(y_sorted, np.nan, dtype=float)
    edge_inflow_right_normal = np.full_like(y_sorted, np.nan, dtype=float)

    for i, (yy, half) in enumerate(zip(y_sorted, xh_clipped)):
        h_eta = yy - float(eta_origin_y_m)
        Gr_h = (float(g) * float(beta) * theta_line * h_eta**3 / nu**2) if h_eta > 0.0 and np.isfinite(theta_line) and np.isfinite(nu_htff26) and nu_htff26 > 0.0 else np.nan
        if not (np.isfinite(half) and half > 0.0 and np.isfinite(Gr_h) and Gr_h > 0.0):
            rows.append({
                "enabled": "false",
                "reason": "invalid eta half-width or local Grashof number at this height",
                "height_above_wire_m": float(yy - wire_y_m),
                "y_m": float(yy),
                "eta_half_width": float(eta_half_width),
                "eta_origin_y_m": float(eta_origin_y_m),
                "eta_edge_half_width_m": float(half) if np.isfinite(half) else np.nan,
                "Gr_h": float(Gr_h) if np.isfinite(Gr_h) else np.nan,
            })
            continue

        xx = np.linspace(-float(half), float(half), max(3, int(n_x)))
        yyline = np.full_like(xx, float(yy))
        ux = finite_or_nan(uxi(xx, yyline))
        uy = finite_or_nan(uyi(xx, yyline))
        mask = np.isfinite(xx) & np.isfinite(ux) & np.isfinite(uy)
        if np.count_nonzero(mask) >= 2:
            x_valid = xx[mask]
            uy_valid = uy[mask]
            mass_signed[i] = robust_trapz(float(rho) * uy_valid, x_valid)
            mass_upward[i] = robust_trapz(np.where(uy_valid > 0.0, float(rho) * uy_valid, 0.0), x_valid)
            mass_downward[i] = robust_trapz(np.where(uy_valid < 0.0, float(rho) * uy_valid, 0.0), x_valid)
            volume_signed[i] = robust_trapz(uy_valid, x_valid)
            mom_positive[i] = robust_trapz(float(rho) * uy_valid * uy_valid, x_valid)

        xleft = np.asarray([-float(half)], dtype=float)
        xright = np.asarray([float(half)], dtype=float)
        yedge = np.asarray([float(yy)], dtype=float)
        ulx = float(finite_or_nan(uxi(xleft, yedge))[0])
        urx = float(finite_or_nan(uxi(xright, yedge))[0])
        uly = float(finite_or_nan(uyi(xleft, yedge))[0])
        ury = float(finite_or_nan(uyi(xright, yedge))[0])
        edge_ux_left[i] = ulx
        edge_ux_right[i] = urx
        edge_uy_left[i] = uly
        edge_uy_right[i] = ury
        # Positive means into the plume core: left side +ux, right side -ux.
        edge_inflow_left_vertical_edge[i] = ulx
        edge_inflow_right_vertical_edge[i] = -urx
        slope = dxh_dy[i]
        denom = math.sqrt(1.0 + slope * slope) if np.isfinite(slope) else np.nan
        if np.isfinite(denom) and denom > 0.0:
            # Inward normal velocities for boundaries x=-xh(y) and x=+xh(y).
            edge_inflow_left_normal[i] = (ulx + uly * slope) / denom
            edge_inflow_right_normal[i] = (-urx + ury * slope) / denom
        ...
        rows.append({
            ...
            "eta_half_width": float(eta_half_width),
            "eta_origin_y_m": float(eta_origin_y_m),
            "height_above_eta_origin_m": float(h_eta),
            "eta_edge_half_width_m": float(half),
            "eta_edge_dxdy": float(dxh_dy[i]) if np.isfinite(dxh_dy[i]) else np.nan,
            "Gr_h": float(Gr_h),
            "n_x_samples": int(n_x),
            "mass_flux_signed_kg_per_s_per_m": float(mass_signed[i]) if np.isfinite(mass_signed[i]) else np.nan,
            "mass_flux_upward_kg_per_s_per_m": float(mass_upward[i]) if np.isfinite(mass_upward[i]) else np.nan,
            "mass_flux_downward_kg_per_s_per_m": float(mass_downward[i]) if np.isfinite(mass_downward[i]) else np.nan,
            "volume_flux_signed_m2_per_s": float(volume_signed[i]) if np.isfinite(volume_signed[i]) else np.nan,
            "vertical_momentum_flux_positive_N_per_m": float(mom_positive[i]) if np.isfinite(mom_positive[i]) else np.nan,
            "edge_ux_left_m_per_s": ulx,
            "edge_ux_right_m_per_s": urx,
            "edge_uy_left_m_per_s": uly,
            "edge_uy_right_m_per_s": ury,
            "edge_inflow_left_vertical_edge_m_per_s": float(edge_inflow_left_vertical_edge[i]) if np.isfinite(edge_inflow_left_vertical_edge[i]) else np.nan,
            "edge_inflow_right_vertical_edge_m_per_s": float(edge_inflow_right_vertical_edge[i]) if np.isfinite(edge_inflow_right_vertical_edge[i]) else np.nan,
            "edge_inflow_mean_vertical_edge_m_per_s": float(0.5 * (edge_inflow_left_vertical_edge[i] + edge_inflow_right_vertical_edge[i])) if np.isfinite(edge_inflow_left_vertical_edge[i]) and np.isfinite(edge_inflow_right_vertical_edge[i]) else np.nan,
            "edge_inflow_left_normal_m_per_s": float(edge_inflow_left_normal[i]) if np.isfinite(edge_inflow_left_normal[i]) else np.nan,
            "edge_inflow_right_normal_m_per_s": float(edge_inflow_right_normal[i]) if np.isfinite(edge_inflow_right_normal[i]) else np.nan,
            "edge_inflow_mean_normal_m_per_s": float(0.5 * (edge_inflow_left_normal[i] + edge_inflow_right_normal[i])) if np.isfinite(edge_inflow_left_normal[i]) and np.isfinite(edge_inflow_right_normal[i]) else np.nan,
        })

    dM_dy = finite_difference_sorted(y_sorted, mass_signed)
    dV_dy = finite_difference_sorted(y_sorted, volume_signed)
    dMom_dy = finite_difference_sorted(y_sorted, mom_positive)
    # Update existing enabled rows with derivative-based entrainment diagnostics.
    by_y = {float(r.get("y_m", np.nan)): r for r in rows if str(r.get("enabled", "")) == "true"}
    for i, yy in enumerate(y_sorted):
        r = by_y.get(float(yy))
        if r is None:
            continue
        r["d_mass_flux_dy_kg_per_s_per_m2"] = float(dM_dy[i]) if np.isfinite(dM_dy[i]) else np.nan
        r["d_volume_flux_dy_m_per_s"] = float(dV_dy[i]) if np.isfinite(dV_dy[i]) else np.nan
        r["d_vertical_momentum_flux_dy_N_per_m2"] = float(dMom_dy[i]) if np.isfinite(dMom_dy[i]) else np.nan
        r["effective_entrainment_velocity_from_mass_m_per_s"] = float(0.5 * dM_dy[i] / float(rho)) if np.isfinite(dM_dy[i]) and float(rho) != 0.0 else np.nan
        r["effective_entrainment_velocity_from_volume_m_per_s"] = float(0.5 * dV_dy[i]) if np.isfinite(dV_dy[i]) else np.nan
        edge_mean = float(r.get("edge_inflow_mean_normal_m_per_s", np.nan))
        eff = float(r["effective_entrainment_velocity_from_mass_m_per_s"])
        r["edge_normal_minus_integral_entrainment_m_per_s"] = edge_mean - eff if np.isfinite(edge_mean) and np.isfinite(eff) else np.nan
...
def plot_local_nusselt(path: Path, rows: List[Dict[str, float | str]], *, figsize: Tuple[float, float] = None, show_titles: bool = False) -> None:
    ...
    save_current_figure_full(path, dpi=220)
    plt.close()
...
def plot_xy(path: Path, x, ys: Sequence[Tuple[str, np.ndarray]], xlabel: str, ylabel: str, title: str, semilogy: bool = False, *, figsize: Tuple[float, float] = (3.0, 2.16), show_titles: bool = False, dpi: int = 220) -> None:
    ...
    if len(ys) > 1:
        place_legend_outside_right()
    save_current_figure_full(path, dpi=dpi)
    plt.close()
...
def compute_entrainment_similarity_scaling(
    entrainment_rows: List[Dict[str, float | int | str]],
    y0: float,
    hmin: Optional[float] = None,
    hmax: Optional[float] = None,
) -> Tuple[List[Dict[str, float | int | str]], List[Dict[str, float | int | str]]]:
    """
    Compare the fixed-|eta| plume-core mass/volume flux with the classical
    self-similar laminar line-plume scaling.

    For a plane laminar line plume:
        M(y)       ~ (y-y0)^(3/5)
        dM/dy      ~ (y-y0)^(-2/5)

    The same exponents apply to volume flux when rho is constant.
    Entrainment-rate fits use only positive derivative values, because a
    log-log fit of dM/dy is not meaningful after recirculation makes the
    effective entrainment negative.
    """
    valid = [r for r in entrainment_rows if str(r.get("enabled", "false")).lower() == "true"]

    if not valid:
        return ([{
            "enabled": "false",
            "reason": "No enabled entrainment rows available for scaling analysis.",
            "y0_m": float(y0),
        }], [])

    y = np.array([float(r.get("y_m", np.nan)) for r in valid], dtype=float)
    h = y - float(y0)

    M = np.array([float(r.get("mass_flux_signed_kg_per_s_per_m", np.nan)) for r in valid], dtype=float)
    V = np.array([float(r.get("volume_flux_signed_m2_per_s", np.nan)) for r in valid], dtype=float)
    dMdy = np.array([float(r.get("d_mass_flux_dy_kg_per_s_per_m2", np.nan)) for r in valid], dtype=float)
    dVdy = np.array([float(r.get("d_volume_flux_dy_m_per_s", np.nan)) for r in valid], dtype=float)

    positive_h = np.isfinite(h) & (h > 0.0)
    if hmin is None:
        hmin_eff = float(np.nanmin(h[positive_h])) if np.any(positive_h) else np.nan
    else:
        hmin_eff = float(hmin)

    # Default upper bound: last point before the integral entrainment becomes
    # non-positive. This targets the part where a self-similar entraining plume
    # comparison is meaningful and avoids the recirculation-dominated branch.
    positive_ent = positive_h & np.isfinite(dMdy) & (dMdy > 0.0)
    if hmax is None:
        hmax_eff = float(np.nanmax(h[positive_ent])) if np.any(positive_ent) else (
            float(np.nanmax(h[positive_h])) if np.any(positive_h) else np.nan
        )
    else:
        hmax_eff = float(hmax)
    ...
    def add_fit(quantity_name: str, values: np.ndarray, target_exponent: float, positive_only: bool = True) -> None:
        mask = np.isfinite(h) & np.isfinite(values) & (h > 0.0)
        if positive_only:
            mask &= values > 0.0
        mask &= h >= hmin_eff
        mask &= h <= hmax_eff

        fit_free = fit_loglog_powerlaw(h[mask], values[mask], hmin_eff, hmax_eff)
        fit_fixed = fit_fixed_exponent_powerlaw(h[mask], values[mask], target_exponent, hmin_eff, hmax_eff)

        summary_rows.append({
            "enabled": "true",
            "quantity": quantity_name,
            "y0_m": float(y0),
            "fit_hmin_m": float(hmin_eff),
            "fit_hmax_m": float(hmax_eff),
            "target_exponent": float(target_exponent),
            "free_exponent": float(fit_free.get("exponent", np.nan)),
            "free_prefactor": float(fit_free.get("C", np.nan)),
            "free_r2": float(fit_free.get("r2", np.nan)),
            "fixed_exponent": float(target_exponent),
            "fixed_prefactor": float(fit_fixed.get("C", np.nan)),
            "fixed_r2": float(fit_fixed.get("r2", np.nan)),
            "npoints": int(fit_free.get("npoints", 0)),
        })

    add_fit("mass_flux_signed_kg_per_s_per_m", M, 3.0 / 5.0, positive_only=True)
    add_fit("volume_flux_signed_m2_per_s", V, 3.0 / 5.0, positive_only=True)
    add_fit("d_mass_flux_dy_kg_per_s_per_m2", dMdy, -2.0 / 5.0, positive_only=True)
    add_fit("d_volume_flux_dy_m_per_s", dVdy, -2.0 / 5.0, positive_only=True)

    point_rows: List[Dict[str, float | int | str]] = []
    for i, r in enumerate(valid):
        hi = float(h[i]) if np.isfinite(h[i]) else np.nan
        in_window = bool(np.isfinite(hi) and np.isfinite(hmin_eff) and np.isfinite(hmax_eff) and hi >= hmin_eff and hi <= hmax_eff)
        point_rows.append({
            "y_m": float(y[i]) if np.isfinite(y[i]) else np.nan,
            "height_above_scaling_origin_m": hi,
            "y0_m": float(y0),
            "mass_flux_signed_kg_per_s_per_m": float(M[i]) if np.isfinite(M[i]) else np.nan,
            "volume_flux_signed_m2_per_s": float(V[i]) if np.isfinite(V[i]) else np.nan,
            "d_mass_flux_dy_kg_per_s_per_m2": float(dMdy[i]) if np.isfinite(dMdy[i]) else np.nan,
            "d_volume_flux_dy_m_per_s": float(dVdy[i]) if np.isfinite(dVdy[i]) else np.nan,
            "used_in_fit_window": "true" if in_window else "false",
            "fit_hmin_m": float(hmin_eff) if np.isfinite(hmin_eff) else np.nan,
            "fit_hmax_m": float(hmax_eff) if np.isfinite(hmax_eff) else np.nan,
        })

    return summary_rows, point_rows


def plot_entrainment_loglog_scaling(
    path: Path,
    point_rows: List[Dict[str, float | int | str]],
    summary_rows: List[Dict[str, float | int | str]],
    quantity: str,
    ...
    title: str,
    *,
    figsize: Tuple[float, float] = (3.4, 2.45),
    show_titles: bool = False,
    dpi: int = 220,
) -> None:
    """Save a log-log plot with computed data, a free-slope fit, and the self-similar reference slope."""
    if not point_rows:
        return

    h = np.array([float(r.get("height_above_scaling_origin_m", np.nan)) for r in point_rows], dtype=float)
    q = np.array([float(r.get(quantity, np.nan)) for r in point_rows], dtype=float)
    hmin_arr = np.array([float(r.get("fit_hmin_m", np.nan)) for r in point_rows], dtype=float)
    hmax_arr = np.array([float(r.get("fit_hmax_m", np.nan)) for r in point_rows], dtype=float)

    mask = np.isfinite(h) & np.isfinite(q) & (h > 0.0) & (q > 0.0)
    if np.count_nonzero(mask) < 2:
        return

    hmin = float(np.nanmedian(hmin_arr[np.isfinite(hmin_arr)])) if np.any(np.isfinite(hmin_arr)) else np.nan
    hmax = float(np.nanmedian(hmax_arr[np.isfinite(hmax_arr)])) if np.any(np.isfinite(hmax_arr)) else np.nan

    fit_row = None
    for r in summary_rows:
        if str(r.get("quantity", "")) == quantity:
            fit_row = r
            break

    fig, ax = plt.subplots(figsize=figsize)
    ax.loglog(h[mask], q[mask], "o", markersize=3.5, label="computed")

    if fit_row is not None:
        C_free = float(fit_row.get("free_prefactor", np.nan))
        n_free = float(fit_row.get("free_exponent", np.nan))
        C_fixed = float(fit_row.get("fixed_prefactor", np.nan))

        hfit_min = hmin if np.isfinite(hmin) else float(np.nanmin(h[mask]))
        hfit_max = hmax if np.isfinite(hmax) else float(np.nanmax(h[mask]))

        if np.isfinite(hfit_min) and np.isfinite(hfit_max) and hfit_max > hfit_min:
            hh = np.geomspace(hfit_min, hfit_max, 100)
            if np.isfinite(C_free) and np.isfinite(n_free):
                ax.loglog(hh, C_free * hh**n_free, "-", label=fr"free fit: $n={n_free:.3f}$")
            if np.isfinite(C_fixed):
                ax.loglog(hh, C_fixed * hh**target_exponent, "--", label=fr"self-similar: $n={target_exponent:.3f}$")
            ax.axvspan(hfit_min, hfit_max, alpha=0.08, label="fit window")

    ax.set_xlabel(r"height above scaling origin [m]")
    ax.set_ylabel(ylabel)
    maybe_set_ax_title(ax, title, show_titles)
    ax.grid(True, which="both", alpha=0.3)
    place_legend_outside_right(ax, fontsize=8)
    save_figure_full(fig, path, dpi=dpi)
    plt.close(fig)
...
def main() -> None:
    ...
    ap.add_argument("--entrainment", action="store_true",
                    help="Compute fixed-|eta| plume-core entrainment diagnostics. This is also done automatically whenever the required eta-scaling inputs are available; the flag is kept as an explicit switch/documentation aid.")
    ap.add_argument("--no-entrainment", action="store_true",
                    help="Disable fixed-|eta| entrainment diagnostics even if the required eta-scaling inputs are available.")
    ap.add_argument("--entrainment-eta-half-width", type=float, default=None,
                    help="Eta half-width used for entrainment diagnostics. Default: --energy-eta-half-width.")
    ap.add_argument("--entrainment-nx", type=int, default=1001,
                    help="Number of horizontal samples for each entrainment mass-flux integral. Default: min(--nx, 1001).")
    ap.add_argument("--entrainment-scaling", action="store_true",
                    help="Fit self-similar laminar line-plume scaling laws to the fixed-|eta| entrainment diagnostics.")
    ap.add_argument("--entrainment-scaling-y0-mode",
                    choices=["wire", "eta_origin", "temperature_virtual_origin", "velocity_virtual_origin"],
                    default="wire",
                    help="Origin used for log-log entrainment scaling fits.")
    ap.add_argument("--entrainment-scaling-hmin", type=float, default=None,
                    help="Minimum height above chosen scaling origin for entrainment scaling fit [m]. Default: smallest valid positive height.")
    ap.add_argument("--entrainment-scaling-hmax", type=float, default=None,
                    help="Maximum height above chosen scaling origin for entrainment scaling fit [m]. Default: last height with positive dM/dy.")
    ...
    ap.add_argument("--plot-width-inch", type=float, default=None,
                    help="Width of saved plot figures in inches. Default: 6.4 in, giving a 16:10 canvas.")
    ap.add_argument("--plot-height-inch", type=float, default=None,
                    help="Optional height of saved plot figures in inches. Default: width * 10/16, i.e. 16:10.")
    ...
    wire_diameter_m = 2.0 * wire_radius_m
    ...
    # Dimensionless variables used in the HTFF26 paper:
    # x*=(x-x_c)/d, y*=(y-y_c)/d, theta=(T-T_inf)/DeltaT,
    # u*=u/u_r, p*=p/(rho*u_r^2), t*=t/(d/u_r).
    # Here d is the cylinder diameter, not the radius/lref.
    htff26_scales = compute_htff26_dimensionless_scales(
        q_input_per_length=args.q_input_per_length,
        k=args.k, mu=args.mu, rho=args.rho, cp=args.cp,
        beta=args.beta, g=args.g, d=wire_diameter_m, T_inf=args.T_inf,
    )
    write_csv(outdir / "dimensionless_scales_htff26.csv", [htff26_scales])
    DeltaT_htff26 = float(htff26_scales["DeltaT_ref_K"])
    uref_htff26 = float(htff26_scales["u_ref_m_per_s"])
    Gr_d_htff26 = float(htff26_scales["Gr_d"])
    Pr_htff26 = float(htff26_scales["Pr"])
    nu_htff26 = float(htff26_scales["nu_m2_per_s"])
    dimless_enabled = bool(htff26_scales.get("enabled", False))
    if not dimless_enabled:
        print("[warning] HTFF26 dimensionless variables disabled: require positive --q-input-per-length, --mu, --rho, --cp, --k, --beta, --g, and --lref.")
    ...
    for h in args.planes:
        ...
        plane_rows.append({
            "height_m": h,
            "height_over_d": h / wire_diameter_m if wire_diameter_m > 0.0 else np.nan,
            "y_m": yp,
            "y_over_d": (yp - wire_y_m) / wire_diameter_m if wire_diameter_m > 0.0 else np.nan,
            "T_center_K": Tc,
            "DeltaT_center_K": Tce,
            "theta_center_HTFF26": Tce / DeltaT_htff26 if dimless_enabled and DeltaT_htff26 != 0.0 else np.nan,
            "uy_center_m_per_s": uyc,
            "uy_center_over_u_r": uyc / uref_htff26 if dimless_enabled and uref_htff26 != 0.0 else np.nan,
            "Gr_h_HTFF26": float(args.g) * float(args.beta) * DeltaT_htff26 * h**3 / nu_htff26**2 if dimless_enabled and h > 0.0 and np.isfinite(nu_htff26) and nu_htff26 > 0.0 else np.nan,
        ...
        for xi, Ti_, uxi_, uyi_, qt_ in zip(xs, Tline, uxline, uyline, qtot_y):
            ...
            profile_rows.append({
                ...
                "y_over_d": (yp - wire_y_m) / wire_diameter_m if wire_diameter_m > 0.0 else np.nan,
                "x_m": xi,
                "x_over_d": (xi - float(args.wire_center_x)) / wire_diameter_m if wire_diameter_m > 0.0 else np.nan,
                "T_K": Ti_,
                "DeltaT_K": dTi_,
                "theta_HTFF26": dTi_ / DeltaT_htff26 if dimless_enabled and DeltaT_htff26 != 0.0 else np.nan,
                "ux_m_per_s": uxi_,
                "uy_m_per_s": uyi_,
                "ux_over_u_r": uxi_ / uref_htff26 if dimless_enabled and uref_htff26 != 0.0 else np.nan,
                "uy_over_u_r": uyi_ / uref_htff26 if dimless_enabled and uref_htff26 != 0.0 else np.nan,
                "qconv_y_W_per_m2": args.rho * args.cp * uyi_ * dTi_,
                "qcond_y_W_per_m2": qt_ - args.rho * args.cp * uyi_ * dTi_,
    ...
    if args.nusselt:
        ...
        plot_local_nusselt(outdir / "wire_nusselt_local.png", nusselt_local_rows, show_titles=args.plot_titles, figsize=plot_figsize)
    ...
    if "boundary_totals" in locals():
        ...
        save_current_figure_full(outdir / "energy_budget_summary.png", dpi=180)
        plt.close()
    ...
    if "entrainment_rows" in locals():
        entrainment_plot_rows = [r for r in entrainment_rows if str(r.get("enabled", "false")) == "true"]
        eh = np.array([r.get("height_above_wire_m", np.nan) for r in entrainment_plot_rows], dtype=float)
        print(f"Plotting entrainment diagnostics for {len(entrainment_plot_rows)} valid rows.")
        plot_xy(outdir / "entrainment_velocity_vs_height.png", eh,
                [("integral: (1/2rho) dM/dy", np.array([r.get("effective_entrainment_velocity_from_mass_m_per_s", np.nan) for r in entrainment_plot_rows], dtype=float)),
                 ("direct edge: mean normal inflow", np.array([r.get("edge_inflow_mean_normal_m_per_s", np.nan) for r in entrainment_plot_rows], dtype=float)),
                 ("direct edge: mean horizontal inflow", np.array([r.get("edge_inflow_mean_vertical_edge_m_per_s", np.nan) for r in entrainment_plot_rows], dtype=float))],
                "height above wire [m]", "entrainment velocity [m/s]",
                "Fixed-eta entrainment velocity diagnostics")
        plot_xy(outdir / "entrainment_mass_flux_derivative.png", eh,
                [("mass flux M(y)", np.array([r.get("mass_flux_signed_kg_per_s_per_m", np.nan) for r in entrainment_plot_rows], dtype=float)),
                 ("dM/dy", np.array([r.get("d_mass_flux_dy_kg_per_s_per_m2", np.nan) for r in entrainment_plot_rows], dtype=float))],
                "height above wire [m]", "mass diagnostic",
                "Fixed-eta mass flux and entrainment rate")
    ...
    def plot_linearized_virtual_origin(path, yy_, amp_, exponent, fit, ylabel, title):
        ...
        plt.grid(True, alpha=0.35)
        place_legend_outside_right()
        save_current_figure_full(path, dpi=180)
        plt.close()
    ...
    plt.grid(True, alpha=0.35)
    place_legend_outside_right()
    save_current_figure_full(outdir / "near_wire_boundary_layer_by_angle.png", dpi=180)
    plt.close()
    ...
    save_current_figure_full(outdir / "near_wire_boundary_layer_polar.png", dpi=180)
    plt.close()

    # Combined profile plots: one figure per quantity, with all requested heights overlaid.
    def combined_profile_plot(filename: str, quantity_key: str, ylabel: str, title: str,
                              x_key: str = "x_m", xlabel: str = "x [m]",
                              use_dimensionless_height_label: bool = False) -> None:
        plt.figure(figsize=plot_figsize)
        for h in args.planes:
            rows = [r for r in profile_rows if abs(r["height_m"] - h) < 1e-15]
            if not rows:
                continue
            xp = np.array([r.get(x_key, np.nan) for r in rows], dtype=float)
            qp = np.array([r[quantity_key] for r in rows], dtype=float)
            if quantity_key in ("ux_m_per_s", "ux_over_u_r"):
                qp = np.abs(qp)
            m = np.isfinite(xp) & np.isfinite(qp)
            if not np.any(m):
                continue
            order = np.argsort(xp[m])
            if use_dimensionless_height_label and wire_diameter_m > 0.0:
                label = fr"$h/d$={h / wire_diameter_m:.4g}"
            else:
                label = f"h={h:g} m"
            plt.plot(xp[m][order], qp[m][order], label=label)
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        maybe_set_title(title, args.plot_titles)
        plt.grid(True, alpha=0.35)
        place_legend_outside_right()
        save_current_figure_full(outdir / filename, dpi=180)
        plt.close()
    ...
    # HTFF26-paper dimensionless profile plots.  These use the paper's finite-cylinder
    # nondimensionalization: x/d, theta=(T-T_inf)/DeltaT, and u/u_r.
    if dimless_enabled:
        combined_profile_plot(
            "profiles_theta_htff26_all_heights.png",
            "theta_HTFF26",
            r"$\theta=(T-T_\infty)/\Delta T$ [-]",
            r"Temperature profiles with HTFF26 nondimensionalization",
            x_key="x_over_d",
            xlabel=r"$(x-x_c)/d$ [-]",
            use_dimensionless_height_label=True,
        )
        combined_profile_plot(
            "profiles_uy_over_ur_htff26_all_heights.png",
            "uy_over_u_r",
            r"$u_y/u_r$ [-]",
            r"Vertical-velocity profiles with HTFF26 nondimensionalization",
            x_key="x_over_d",
            xlabel=r"$(x-x_c)/d$ [-]",
            use_dimensionless_height_label=True,
        )
        combined_profile_plot(
            "profiles_ux_over_ur_htff26_all_heights.png",
            "ux_over_u_r",
            r"$u_x/u_r$ [-]",
            r"Horizontal-velocity profiles with HTFF26 nondimensionalization",
            x_key="x_over_d",
            xlabel=r"$(x-x_c)/d$ [-]",
            use_dimensionless_height_label=True,
        )
    ...
    # eta = (x_transverse / h) * Gr_h^(1/5),
    # Gr_h = g beta DeltaT h^3 / nu^2, with DeltaT = Qdot/(k Pr) = Qdot/(mu cp).
    # The same rows also contain the paper-style similarity-scaled ordinates:
    #   theta Gr_h^(1/5)
    #   (u_y/u_r) / [(nu/(h u_r)) Gr_h^(2/5)]
    eta_rows = []
    ...
    if eta_enabled and nu > 0.0 and theta_line_source > 0.0:
        for r in profile_rows:
            ...
            eta_rows.append({
                **r,
                ...
                "DeltaT_ref_HTFF26_K": DeltaT_htff26 if dimless_enabled else theta_line_source,
                "Theta_line_source_K": theta_line_source,
                "nu_m2_per_s": nu,
                "u_ref_HTFF26_m_per_s": uref_htff26 if dimless_enabled else np.nan,
                "Gr_x": Gr_h,
                "Gr_h_HTFF26": Gr_h,
                "theta_Gr_h_1_5_HTFF26": (float(r["DeltaT_K"]) / DeltaT_htff26) * Gr_h**0.2 if dimless_enabled and np.isfinite(Gr_h) and Gr_h > 0.0 and DeltaT_htff26 != 0.0 else np.nan,
                "uy_over_ur_similarity_HTFF26": (float(r["uy_m_per_s"]) / uref_htff26) / ((nu / (h_eta * uref_htff26)) * Gr_h**0.4) if dimless_enabled and h_eta > 0.0 and np.isfinite(Gr_h) and Gr_h > 0.0 and uref_htff26 != 0.0 else np.nan,
        ...
        def combined_eta_plot(filename: str, quantity_key: str, ylabel: str, title: str) -> None:
            ...
            plt.grid(True, alpha=0.35)
            place_legend_outside_right()
            save_current_figure_full(outdir / filename, dpi=220)
            plt.close()
    ...
    # Fixed-|eta| entrainment diagnostics.
    # Positive entrainment means inflow into the selected plume core. The derivative
    # estimate is based on M(y)=int rho*u_y dx over |eta|<=eta_e; the edge estimates
    # sample the lateral velocity on the moving sidewalls x=+-x_e(y).
    entrainment_rows: List[Dict[str, float | int | str]] = []
    do_entrainment = (not getattr(args, "no_entrainment", False)) and eta_enabled
    if do_entrainment and args.mu is not None and args.beta is not None and args.q_input_per_length is not None:
        eta_hw_ent = float(args.entrainment_eta_half_width) if args.entrainment_eta_half_width is not None else float(args.energy_eta_half_width)
        ent_nx = int(args.entrainment_nx) if args.entrainment_nx is not None else int(min(max(201, args.nx), 1001))
        try:
            entrainment_rows = compute_fixed_eta_entrainment_diagnostics(
                Ti=Ti,
                uxi=uxi,
                uyi=uyi,
                y_levels=yy,
                eta_half_width=eta_hw_ent,
                eta_origin_y_m=eta_origin_y_m,
                wire_y_m=wire_y_m,
                xmin=xmin,
                xmax=xmax,
                ...
                g=float(args.g),
                q_input_per_length=float(args.q_input_per_length),
                n_x=ent_nx,
            )
        except Exception as exc:
            entrainment_rows = [{
                ...
                "eta_half_width": eta_hw_ent,
                "eta_origin_y_m": eta_origin_y_m,
                "description": "Fixed-|eta| entrainment diagnostics failed during integration.",
            }]
    else:
        missing = []
        if args.mu is None:
        ...
        if getattr(args, "no_entrainment", False):
            reason = "disabled by --no-entrainment"
        else:
            reason = "requires eta scaling inputs" + (": " + ", ".join(missing) if missing else "")
        entrainment_rows = [{
            "enabled": "false",
            "reason": reason,
            "eta_half_width": float(args.entrainment_eta_half_width) if args.entrainment_eta_half_width is not None else float(args.energy_eta_half_width),
            "description": "No entrainment diagnostics were computed.",
        }]
    write_csv(outdir / "entrainment_diagnostics.csv", entrainment_rows)

    # Optional comparison with classical self-similar laminar line-plume scaling.
    # Fujii/Gebhart-type plane-plume similarity gives:
    #   mass/volume flow rate ~ h^(3/5)
    #   entrainment rate dM/dh ~ h^(-2/5)
    entrainment_scaling_rows: List[Dict[str, float | int | str]] = []
    entrainment_scaling_point_rows: List[Dict[str, float | int | str]] = []
    scaling_y0 = float(wire_y_m)

    if getattr(args, "entrainment_scaling", False):
        if args.entrainment_scaling_y0_mode == "wire":
            scaling_y0 = float(wire_y_m)
        elif args.entrainment_scaling_y0_mode == "eta_origin":
            scaling_y0 = float(eta_origin_y_m)
        elif args.entrainment_scaling_y0_mode == "temperature_virtual_origin":
            scaling_y0 = float(fitT.get("y0", np.nan))
        elif args.entrainment_scaling_y0_mode == "velocity_virtual_origin":
            scaling_y0 = float(fitU.get("y0", np.nan))

        if not np.isfinite(scaling_y0):
            entrainment_scaling_rows = [{
                "enabled": "false",
                "reason": "Selected scaling origin is not finite.",
                "scaling_y0_mode": str(args.entrainment_scaling_y0_mode),
                "y0_m": scaling_y0,
            }]
            entrainment_scaling_point_rows = []
        else:
            entrainment_scaling_rows, entrainment_scaling_point_rows = compute_entrainment_similarity_scaling(
                entrainment_rows=entrainment_rows,
                y0=scaling_y0,
                hmin=args.entrainment_scaling_hmin,
                hmax=args.entrainment_scaling_hmax,
            )

        for r in entrainment_scaling_rows:
            r["scaling_y0_mode"] = str(args.entrainment_scaling_y0_mode)
        for r in entrainment_scaling_point_rows:
            r["scaling_y0_mode"] = str(args.entrainment_scaling_y0_mode)

        write_csv(outdir / "entrainment_similarity_scaling_fits.csv", entrainment_scaling_rows)
        write_csv(outdir / "entrainment_similarity_scaling_points.csv", entrainment_scaling_point_rows)

        if entrainment_scaling_point_rows:
            plot_entrainment_loglog_scaling(
                outdir / "entrainment_mass_flux_loglog_scaling.png",
                entrainment_scaling_point_rows, entrainment_scaling_rows,
                "mass_flux_signed_kg_per_s_per_m",
                r"$M(y)$ [kg s$^{-1}$ m$^{-1}$]",
                3.0 / 5.0,
                r"Fixed-$|\eta|$ mass flux: self-similar scaling",
                figsize=plot_figsize, show_titles=args.plot_titles, dpi=220,
            )
            plot_entrainment_loglog_scaling(
                outdir / "entrainment_volume_flux_loglog_scaling.png",
                entrainment_scaling_point_rows, entrainment_scaling_rows,
                "volume_flux_signed_m2_per_s",
                r"$\dot{V}(y)$ [m$^2$ s$^{-1}$]",
                3.0 / 5.0,
                r"Fixed-$|\eta|$ volume flux: self-similar scaling",
                figsize=plot_figsize, show_titles=args.plot_titles, dpi=220,
            )
            plot_entrainment_loglog_scaling(
                outdir / "entrainment_mass_rate_loglog_scaling.png",
                entrainment_scaling_point_rows, entrainment_scaling_rows,
                "d_mass_flux_dy_kg_per_s_per_m2",
                r"$dM/dy$ [kg s$^{-1}$ m$^{-2}$]",
                -2.0 / 5.0,
                r"Mass entrainment rate: self-similar scaling",
                figsize=plot_figsize, show_titles=args.plot_titles, dpi=220,
            )
            plot_entrainment_loglog_scaling(
                outdir / "entrainment_volume_rate_loglog_scaling.png",
                entrainment_scaling_point_rows, entrainment_scaling_rows,
                "d_volume_flux_dy_m_per_s",
                r"$d\dot{V}/dy$ [m s$^{-1}$]",
                -2.0 / 5.0,
                r"Volume entrainment rate: self-similar scaling",
                figsize=plot_figsize, show_titles=args.plot_titles, dpi=220,
            )

    if entrainment_rows:
        entrainment_df = pd.DataFrame(entrainment_rows)
        required_entrainment_cols = [
            "height_above_wire_m",
            "effective_entrainment_velocity_from_mass_m_per_s",
            "edge_inflow_mean_vertical_edge_m_per_s",
            "edge_inflow_mean_normal_m_per_s",
            "d_mass_flux_dy_kg_per_s_per_m2",
        ]
        has_entrainment_plot_data = all(c in entrainment_df.columns for c in required_entrainment_cols)

        if has_entrainment_plot_data:
            entrainment_plot_df = entrainment_df[required_entrainment_cols].apply(pd.to_numeric, errors="coerce")
            entrainment_plot_df = entrainment_plot_df.dropna(subset=["height_above_wire_m"])

            if not entrainment_plot_df.empty:
                h = entrainment_plot_df["height_above_wire_m"].to_numpy()

                plt.figure(figsize=plot_figsize)
                plt.plot(
                    h,
                    entrainment_plot_df["effective_entrainment_velocity_from_mass_m_per_s"],
                    label="from mass-flux derivative",
                )
                plt.plot(
                    h,
                    entrainment_plot_df["edge_inflow_mean_vertical_edge_m_per_s"],
                    label="mean vertical-edge inflow",
                )
                plt.plot(
                    h,
                    entrainment_plot_df["edge_inflow_mean_normal_m_per_s"],
                    label="mean normal-edge inflow",
                )
                ...
                save_current_figure_full(outdir / "entrainment_velocity_vs_height.png", dpi=220)
                plt.close()

                plt.figure(figsize=plot_figsize)
                plt.plot(
                    h,
                    entrainment_plot_df["d_mass_flux_dy_kg_per_s_per_m2"],
                    label="dM/dy",
                )
                plt.axhline(0.0, linestyle="--", linewidth=1)
                plt.xlabel("Height above wire [m]")
                ...
                place_legend_outside_right()
                plt.grid(True, alpha=0.3)
                save_current_figure_full(outdir / "entrainment_mass_flux_derivative.png", dpi=220)
                plt.close()
    ...
    if eta_enabled and eta_rows:
        ...
        if fixed_eta_mass_momentum_rows:
            ...
            save_figure_full(fig, outdir / "fixed_eta_mass_momentum_fluxes.png", dpi=220)
            plt.close(fig)
    else:
        fixed_eta_mass_momentum_rows.append({
            "enabled": False,
    ...
    def plot_profile_comparison(quantity_key: str, ylabel: str, filename_prefix: str, title_prefix: str) -> None:
        for h in args.planes:
            ...
            save_current_figure_full(outdir / f"{filename_prefix}_h_{safe_h}m_comparison.png", dpi=220)
            plt.close()
    ...
    with summary_path.open("w") as f:
        ...
        f.write(f"Wire diameter d = 2*lref: {wire_diameter_m:.16e} m\n")
        f.write("\nHTFF26 dimensionless scales:\n")
        f.write("  x*=(x-x_c)/d, y*=(y-y_c)/d, theta=(T-T_inf)/DeltaT, u*=u/u_r, p*=p/(rho*u_r^2)\n")
        f.write(f"  DeltaT = Qdot/(k Pr): {DeltaT_htff26:.8e} K\n")
        f.write(f"  u_r = sqrt(g*d*beta*DeltaT): {uref_htff26:.8e} m/s\n")
        f.write(f"  Pr = {Pr_htff26:.8e}, Gr_d = {Gr_d_htff26:.8e}, Ec_tilde = {float(htff26_scales['Ec_tilde']):.8e}\n")
        ...
        if getattr(args, "entrainment_scaling", False):
            f.write("\nEntrainment self-similar scaling check:\n")
            f.write("  Classical laminar line-plume prediction: M ~ h^(3/5), dM/dh ~ h^(-2/5).\n")
            f.write(f"  Scaling origin mode: {args.entrainment_scaling_y0_mode}\n")
            try:
                f.write(f"  Scaling origin y0 = {scaling_y0:.8e} m\n")
            except Exception:
                pass
            f.write("  Fit summary written to entrainment_similarity_scaling_fits.csv.\n")
            f.write("  Pointwise log-log data written to entrainment_similarity_scaling_points.csv.\n")
            f.write("  Log-log figures: entrainment_mass_flux_loglog_scaling.png, entrainment_volume_flux_loglog_scaling.png, entrainment_mass_rate_loglog_scaling.png, entrainment_volume_rate_loglog_scaling.png.\n")
        f.write("\nMain outputs:\n")
        f.write("  plane_integrals.csv\n")
        f.write("  plane_profiles.csv\n")
        f.write("  plume_enthalpy_balance.csv\n")
        f.write("  entrainment_diagnostics.csv\n")
        if getattr(args, "entrainment_scaling", False):
            f.write("  entrainment_similarity_scaling_fits.csv\n")
            f.write("  entrainment_similarity_scaling_points.csv\n")
            f.write("  entrainment_*_loglog_scaling.png\n")
